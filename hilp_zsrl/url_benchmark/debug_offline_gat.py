...
# ---- GAT minimal baseline: forward/inverse models + integration into Workspace ----
import os
import json
import math
import tempfile
...
class MLP(nn.Module):
    def __init__(self, in_dim, out_dim, hidden=256, layers=2):
        ...
        for _ in range(layers):
            h += [nn.Linear(d, hidden), nn.ReLU(inplace=True)]
            d = hidden
        h += [nn.Linear(d, out_dim)]
...
class InverseModel(nn.Module):
    """f_inv_sim: (s, s'_target_in_sim) -> a_hat  (在模拟器里产生目标状态效应的动作)"""
    def __init__(self, obs_dim, act_dim, hidden=256, layers=2):
        super().__init__()
        self.core = MLP(obs_dim * 2, act_dim, hidden, layers)
...
class GATModule:
    """
    - 训练：forward 用目标(真实)环境数据，inverse 用模拟环境数据
    - 运行：g(s,a)=f_inv_sim(s, f_real(s,a))
    """
    def __init__(self, obs_dim, act_dim, device, cfg):
        self.device = device
        self.obs_dim = obs_dim
        self.act_dim = act_dim
        self.alpha = float(getattr(cfg, "gat_alpha", 1.0))   # 平滑系数，可为 1.0
        ...
        self.f_inv_sim = InverseModel(obs_dim, act_dim, hidden=hidden, layers=layers).to(device)
        ...
        self.inv_path = self.ckpt_dir / "inverse.pt"

        # 训练超参
        self.epochs = int(getattr(cfg, "gat_epochs", 200))
        self.bs = int(getattr(cfg, "gat_batch_size", 1024))
        self.lr = float(getattr(cfg, "gat_lr", 1e-3))
        self.num_samples_forward = int(getattr(cfg, "gat_forward_samples", 50000))
        self.num_samples_inverse = int(getattr(cfg, "gat_inverse_samples", 50000))

    def _gather_from_buffer(self, replay, max_n, device, use_pixels=False):
        """从 ReplayBuffer 聚合 (obs, act, next_obs) 张量"""
        obs_list, act_list, next_list = [], [], []
        total = 0
        bs = min(self.bs, 4096)
        while total < max_n:
            batch = replay.sample(bs, custom_reward=None).to(device)
            # 假定 batch 包含 obs, action, next_obs
            # 若某些 agent 用 state feature，需要与示例一致选 obs / next_obs
            obs = batch.obs if hasattr(batch, "obs") else batch.next_obs * 0  # fallback
            nxt = batch.next_obs
            act = batch.action if hasattr(batch, "action") else None
            if act is None:
                # 若 buffer 不含 action，就用 agent/环境接口前置采集；此处简单跳过
                total += 0
                continue
            obs_list.append(obs)
            act_list.append(act)
            ...
            total += nxt.size(0)
        if total == 0:
            raise RuntimeError("ReplayBuffer 未返回 action 字段，无法训练 GAT。")
    ...
    def maybe_load(self):
        ...
        if self.inv_path.exists():
            self.f_inv_sim.load_state_dict(torch.load(self.inv_path, map_location=self.device))
            ok = ok and True
        return ok

    def train_models(self, replay_sim, replay_real):
        """训练 forward(in real/target env) & inverse(in sim env)"""
        # 采样数据
        S_r, A_r, Sp_r = self._gather_from_buffer(replay_real, self.num_samples_forward, self.device)
        S_s, A_s, Sp_s = self._gather_from_buffer(replay_sim,  self.num_samples_inverse, self.device)

        # ===== 训练 forward: (s,a)->s' (real) =====
        f_opt = optim.Adam(self.f_real.parameters(), lr=self.lr)
        mse = nn.MSELoss()
        n = S_r.size(0)
        ...
        # ===== 训练 inverse: (s, s'_desired)->a_hat (sim) =====
        # 这里用 real forward 对 Sim 的 (s,a) 产生目标效果 s'_des，然后让 inverse 在 Sim 中回归能产生该 s'_des 的动作
        with torch.no_grad():
            Sp_des = self.f_real(S_s, A_s)  # s'_(real-pred) 作为期望效果
        inv_opt = optim.Adam(self.f_inv_sim.parameters(), lr=self.lr)
        msel = nn.MSELoss()
        n2 = S_s.size(0)
        idx2 = torch.arange(n2, device=self.device)
        for ep in range(self.epochs):
            perm = idx2[torch.randperm(n2)]
            for i in range(0, n2, self.bs):
                j = min(i + self.bs, n2)
                s = S_s[perm[i:j]]
                sp_des = Sp_des[perm[i:j]]
                a_target = A_s[perm[i:j]]
                a_pred = self.f_inv_sim(s, sp_des)
                loss = msel(a_pred, a_target)  # 连续动作回归
                inv_opt.zero_grad()
                loss.backward()
                inv_opt.step()
        ...
        torch.save(self.f_inv_sim.state_dict(), self.inv_path)

    @torch.no_grad()
    def transform_action(self, obs_np, action_np):
        """
        输入 numpy，输出 numpy： a_g = alpha * f_inv(s, f_forward(s,a)) + (1-alpha) * a
        """
        ...
        a_hat = self.f_inv_sim(s, s_real_next)
        a_g = self.alpha * a_hat + (1.0 - self.alpha) * a
        return a_g.squeeze(0).cpu().numpy()


# ====== 替换版 Workspace：仅改动/新增与 GAT 相关的部分，其他逻辑保持原样 ======
class Workspace:
    def __init__(self, cfg: Config) -> None:
        ...
        if self.use_gat:
            ...
            # 观测取 obs（与你的示例一致），动作假设存在 batch.action
            if not hasattr(b, "action"):
                print("[GAT] 警告：ReplayBuffer batch 不包含 action，无法训练 GAT。use_gat=False。")
                self.use_gat = False
            else:
                ...
                if not loaded:
                    print("[GAT] 训练 forward/inverse 模型中 ...")
                    self.gat.train_models(self.replay_loader, self.replay_loader_env_targ)
                    print("[GAT] 训练完成并已保存权重。")
                else:
                    print("[GAT] 已加载已存在的 forward/inverse 权重。")
        ...
        while eval_until_episode(episode):
            ...
            while not time_step.last():
                ...
                with torch.no_grad(), utils.eval_mode(self.agent):
                    ...
                    if self.use_gat:
                        try:
                            action = self.gat.transform_action(time_step.observation, raw_action)
                        except Exception as e:
                            # 避免评估崩溃：失败时退回原始动作
                            print(f"[GAT] transform 失败，使用原始动作。err={e}")
                            action = raw_action
