...
def save_embeddings_for_vis(enc_sim: torch.Tensor,
                            save_dir: str,
                            tag: str):
    """
    只保存原始 phi(s) embeddings，不做任何处理。
    enc_sim: [N, D]
    tag: 比如 'g0', 'g-5', 'g-10', 'g-15'
    """
    os.makedirs(save_dir, exist_ok=True)
    emb_np = enc_sim.detach().cpu().numpy()
    out_path = os.path.join(save_dir, f"phi_{tag}.npy")
    np.save(out_path, emb_np)
    print(f"[save_embeddings_for_vis] saved {emb_np.shape} to {out_path}")
...
#     enc_sim,
#     save_dir,
#     filename="tsne_enc_sim.png",
#     max_points=5000,
#     perplexity=30.0,
#     random_state=0,
# ):
#     """
#     Visualize high-dimensional embeddings with t-SNE and save to a file.

#     Parameters
#     ----------
#     enc_sim : np.ndarray or torch.Tensor
#         Embeddings of shape [N, D] (N samples, D-dim embedding).
#     save_dir : str
#         Directory to save the visualization.
#     filename : str, optional
#         Name of the saved image file, by default "tsne_enc_sim.png".
#     max_points : int, optional
#         Maximum number of points to plot (subsample if N is larger), by default 5000.
#     perplexity : float, optional
#         t-SNE perplexity, by default 30.0.
#     random_state : int, optional
#         Random seed for t-SNE, by default 0.
#     """

#     # -------- 1. 转成 numpy --------
#     if TORCH_AVAILABLE and isinstance(enc_sim, torch.Tensor):
#         emb = enc_sim.detach().cpu().numpy()
#     elif isinstance(enc_sim, np.ndarray):
#         emb = enc_sim
#     else:
#         raise TypeError(
#             f"enc_sim must be np.ndarray or torch.Tensor, got {type(enc_sim)}"
#         )
...
#         emb = emb.reshape(1, -1)
#     elif emb.ndim > 2:
#         # 尝试把后面所有维度 flatten 成 [N, D]
#         # 比如 [T, B, D] -> [T*B, D]
#         n_samples = np.prod(emb.shape[:-1])
#         d = emb.shape[-1]
#         emb = emb.reshape(int(n_samples), int(d))

#     if emb.ndim != 2:
#         raise ValueError(
#             f"Expected enc_sim to be 2D after reshape, got shape {emb.shape}"
#         )

#     N, D = emb.shape
#     if N < 2:
#         raise ValueError(
#             f"Need at least 2 points for t-SNE, got N={N} with shape {emb.shape}"
#         )

#     # -------- 3. 下采样（防止太多点） --------
#     if N > max_points:
#         idx = np.random.RandomState(random_state).choice(N, max_points, replace=False)
#         emb = emb[idx]
#         N = max_points
...
#     )
#     emb_2d = tsne.fit_transform(emb)  # [N, 2]
...
#     plt.colorbar(scatter, label="normalized index")

#     plt.tight_layout()
#     plt.savefig(save_path, dpi=300)
#     plt.close()

#     print(f"[visualize_tsne_embeddings] Saved t-SNE plot to: {save_path}")
...
def visualize_tsne_embeddings(
    ...
    """
    100% 不做插值、不做过滤、不做平滑、不做归一化的 t-SNE 可视化函数。
    唯一的处理：reshape + numpy 转换（可视化需要）。
    """

    # -------- 1. 转 numpy（仅转换 dtype，不改变值） --------
    if TORCH_AVAILABLE and isinstance(enc_sim, torch.Tensor):
        emb = enc_sim.detach().cpu().numpy()
    elif isinstance(enc_sim, np.ndarray):
        emb = enc_sim
    else:
        raise TypeError(f"Expected np.ndarray or torch.Tensor, got {type(enc_sim)}")

    # -------- 2. reshape（保持数值不变，只改变视角） --------
    if emb.ndim == 1:
        emb = emb.reshape(1, -1)
    elif emb.ndim > 2:
        # flatten 维度，不修改任何浮点数
    ...
    # 最终必须是 [N, D]
    if emb.ndim != 2:
        raise ValueError(f"After reshape, expected 2D array, got {emb.shape}")

    N, D = emb.shape

    # -------- 3. 防止太多点：仅下采样（选择样本，不修改任何数据） --------
    if N > max_points:
        idx = np.random.RandomState(random_state).choice(N, max_points, replace=False)
        emb = emb[idx]
        N = max_points
    ...
    plt.figure(figsize=(6,6))
    plt.scatter(emb_2d[:,0], emb_2d[:,1], s=4, alpha=0.8)
    plt.xticks([]); plt.yticks([])
    plt.title("t-SNE (raw embeddings)")
    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
    plt.close()

    print(f"[visualize_tsne_embeddings_raw] Saved to: {save_path}")


def visualize_states_and_embeddings(
    obs,
    enc,
    save_dir,
    filename,
    state_xy_idx=(0, 1),   # obs 的哪两维当成 (x,y)，默认前两维
    max_points=5000,
    perplexity=30,
    random_state=0,
    ...
    """
    画出 (1) 原始状态的二维投影 S, (2) 对应的 phi(s) 的 2D 表示（直接 2D 或 t-SNE）。
    ...
    # ------------ 1. 转成 numpy ------------
    obs_np = _to_numpy(obs)      # [N, ...]
    enc_np = _to_numpy(enc)      # [N, D] or [*, D]

    # ---- reshape，保证都是 [N, *] / [N, D] ----
    if obs_np.ndim > 2:
        obs_np = obs_np.reshape(-1, obs_np.shape[-1])
    if enc_np.ndim > 2:
        enc_np = enc_np.reshape(-1, enc_np.shape[-1])

    assert obs_np.shape[0] == enc_np.shape[0], \
        f"obs and enc must have same N, got {obs_np.shape[0]} vs {enc_np.shape[0]}"

    N = obs_np.shape[0]

    # ------------ 2. 抽取 (x,y) 坐标，用来画左图 & 做颜色 ------------
    x_idx, y_idx = state_xy_idx
    if obs_np.shape[1] <= max(x_idx, y_idx):
        raise ValueError(
            f"state_xy_idx={state_xy_idx} out of range for obs shape {obs_np.shape}"
        )
    xy = obs_np[:, [x_idx, y_idx]]  # [N, 2]
    ...
    # ------------ 4. 颜色映射：根据 (x,y) 位置生成渐变颜色 ------------
    # 归一化到 [0,1]
    x_norm = (xy[:, 0] - xy[:, 0].min()) / max(1e-8, (xy[:, 0].max() - xy[:, 0].min()))
    y_norm = (xy[:, 1] - xy[:, 1].min()) / max(1e-8, (xy[:, 1].max() - xy[:, 1].min()))

    # 用 x+y 决定 hue，y 决定亮度，类似原论文那种彩虹渐变
    t = 0.5 * x_norm + 0.5 * y_norm           # [0,1]
    cmap = plt.get_cmap("hsv")
    colors = cmap(t)

    # ------------ 5. 准备 phi(s) 的 2D 表示（如果 D>2 用 t-SNE） ------------
    D = enc_np.shape[1]
    if D == 2:
        emb_2d = enc_np
    else:
        ...
        emb_2d = tsne.fit_transform(enc_np)   # [N, 2]
    ...
    # 左：状态空间 S
    ax = axes[0]
    ax.scatter(xy[:, 0], xy[:, 1], c=colors, s=4)
    ax.set_title(f"{title_prefix}: States S")
    ax.set_xlabel("state dim {}".format(x_idx))
    ax.set_ylabel("state dim {}".format(y_idx))
    ax.set_aspect("equal", "box")

    # 右：Hilbert embedding phi(s)
    ax = axes[1]
    ax.scatter(emb_2d[:, 0], emb_2d[:, 1], c=colors, s=4)
    ax.set_title(f"{title_prefix}: Embedding $\\phi(s)$ (2D / t-SNE)")
    ax.set_xticks([])
    ax.set_yticks([])
    ...
    print(f"[visualize_states_and_embeddings] Saved to: {save_path}")
...
class SFAgentConfig:
    ...
    # added value: 
    # jajaja:
    lambda_wls: float = 4           # 加权最小二乘里真实域权重 λ>1
    use_adapter: bool = True          # 是否启用 DeepSet + Adapter
    adapter_steps: int = 1000 # 1 - 40
...
class SFAgent:
    ...
    # TODO: Ablation - initial solution: 
    # def infer_meta_from_obs_and_rewards_sim2real(
    #     self,
    ...
    # TODO:  Ablation - dyna solution: 
    # def infer_meta_from_obs_and_rewards_sim2real(
    #         self,
    #         obs: torch.Tensor,          # 模拟域 N×...   (不用)
    #         reward: torch.Tensor,       # 模拟域 N×1 or N (不用)
    #         next_obs: torch.Tensor,     # 模拟域 N×...   (不用)

    #         obs_targ: torch.Tensor,     # 真实域 M×...
    #         reward_targ: torch.Tensor,  # 真实域 M×1 or M
    #         next_obs_targ: torch.Tensor,# 真实域 M×...
    #         vis: bool = False
    #     ):
    #     """
    #     Ablation：仅用目标域的动态签名进行推理期自适应
    #     - 不做 WLS，不计算 z_src
    #     - DeepSet(phi(state_targ)) -> η
    #     - Adapter 仅训练 1 个 epoch，得到 z_final
    #     - 返回 meta["z"]：L2 归一化并乘 sqrt(D)
    #     """
    #     from collections import OrderedDict
    #     import math
    #     import torch.nn.functional as F
    ...
    #     # helper: 从编码特征构造 Phi（与原 feature_type 逻辑一致）
    #     def build_phi(encoded, encoded_next):
    #         if self.cfg.feature_type == "state":
    #             return self.feature_learner.feature_net(encoded)  # [M, D]
    #         elif self.cfg.feature_type == "diff":
    #             return (self.feature_learner.feature_net(encoded_next)
    #                     - self.feature_learner.feature_net(encoded))  # [M, D]
    #         else:  # "concat"
    #             return torch.cat(
    #                 [self.feature_learner.feature_net(encoded),
    #                 self.feature_learner.feature_net(encoded_next)],
    #                 dim=-1
    #             )  # 要保证 concat 后维度与 cfg.z_dim 对齐
    ...
    #     # --------------------------
    #     # 1) DeepSet 统计 η（置换不变的动态签名）
    #     #    即便 feature_type=="diff"，这里建议用 "state" 表征统计环境分布
    #     # --------------------------
    #     with torch.no_grad():
    #         phi_state_targ = self.feature_learner.feature_net(enc_targ)  # M x D_state
    #         eta = self.deepset(phi_state_targ)                           # K 维或 1 x K
    #         if eta.dim() == 1:
    #             eta = eta.unsqueeze(0)                                   # 1 x K

    #     # --------------------------
    #     # 2) Adapter 只训练 1 个 epoch
    #     #    为兼容现有 adapter(z, eta) 接口，这里用全零 z 作为占位
    #     # --------------------------
    #     z_zero = torch.zeros(1, D, device=device)  # 占位 z，不包含任何先验

    #     targ_ds = TensorDataset(Phi_targ.detach(), r_targ.detach())
    #     bs = int(getattr(self.cfg, "adapter_batch_size", 256))
    #     targ_dl = DataLoader(targ_ds, batch_size=min(bs, len(targ_ds)),
    #                         shuffle=True, drop_last=False)

    #     self.adapter.train()
    #     lr = float(getattr(self.cfg, "adapter_lr", 1e-3))
    #     optimizer = torch.optim.Adam(self.adapter.parameters(), lr=lr)

    #     # exactly 1 epoch
    #     for Phi_b, r_b in targ_dl:
    #         optimizer.zero_grad()
    #         # 若你的 adapter 支持只以 η 为输入，可改为：z_refined = self.adapter(eta)
    #         z_refined = self.adapter(z_zero, eta)            # 1 x D
    ...
    #     self.adapter.eval()
    #     with torch.no_grad():
    #         z_final = self.adapter(z_zero, eta).squeeze(0)    # D,
    #         z_final = math.sqrt(D) * F.normalize(z_final, dim=0)

    #     meta = OrderedDict()
    #     meta["z"] = z_final.detach().cpu().numpy()
    #     return meta

    # TODO: Ablation - dyna + init solution:  
    # def infer_meta_from_obs_and_rewards_sim2real(
    #         self,
    #         obs: torch.Tensor,          # 模拟域 N×...
    #         reward: torch.Tensor,       # 模拟域 N×1 or N
    #         next_obs: torch.Tensor,     # 模拟域 N×...
    ...
    #     ):
    #     """
    #     Minimal adaptation (1-epoch-1-batch):
    #     (a) WLS 求 z_src（融合 sim+real）
    #     (b) DeepSet(phi(state_targ)) -> η
    #     (c) Adapter 仅更新 1 个 batch（极简适配），得 z_final
    #     返回 meta["z"]: L2 归一化并乘 sqrt(D)
    #     """
    #     from collections import OrderedDict
    #     import math
    ...
    #     # helper: 构造 Φ（与现有 feature_type 对齐）
    #     def build_phi(encoded, encoded_next):
    #         if self.cfg.feature_type == "state":
    #             return self.feature_learner.feature_net(encoded)  # [*, D]
    #         elif self.cfg.feature_type == "diff":
    ...
    #     # --------------------------
    #     # 1) 加权联合最小二乘 -> z_src
    #     #    A = [Phi_sim; sqrt(lambda)*Phi_targ], b = [r_sim; sqrt(lambda)*r_targ]
    #     # --------------------------
    #     lam = float(getattr(self.cfg, "lambda_wls", 4.0))
    #     lam_sqrt = math.sqrt(lam)
    #     with torch.no_grad():
    ...
    #         z_src = z_src.squeeze(-1)                               # D
    #         z_src = math.sqrt(D) * F.normalize(z_src, dim=0)        # 规范化到论文尺度
    ...
    #     with torch.no_grad():
    #         phi_state_targ = self.feature_learner.feature_net(enc_targ)  # M x D_state
    #         eta = self.deepset(phi_state_targ)                           # K 或 1 x K
    #         if eta.dim() == 1:
    #             eta = eta.unsqueeze(0)                                   # 1 x K

    #     # --------------------------
    #     # 3) Adapter 极简适配：仅 1 epoch 且只 1 个 batch
    #     #    目标：尽量模拟“几乎无适配”，但保留合并 z_src + η 的形式
    #     # --------------------------
    #     z_src_tensor = z_src.detach().unsqueeze(0)  # 1 x D

    #     targ_ds = TensorDataset(Phi_targ.detach(), r_targ.detach())
    #     bs = min(int(getattr(self.cfg, "adapter_batch_size", 256)), len(targ_ds))
    #     targ_dl = DataLoader(targ_ds, batch_size=bs, shuffle=True, drop_last=False)

    #     self.adapter.train()
    #     lr = float(getattr(self.cfg, "adapter_lr", 1e-3))
    #     optimizer = torch.optim.Adam(self.adapter.parameters(), lr=lr)

    #     # 只更新第一个 batch（1-epoch-1-batch）
    #     for Phi_b, r_b in targ_dl:
    #         optimizer.zero_grad()
    ...
    # # TODO: today update - Sep.8: hahahahahha: ours ready - March 1st, Camera-revalidate (ours)
    # def infer_meta_from_obs_and_rewards_sim2real(
    #     self,
    #     obs: torch.Tensor,          # 模拟域 N×...   (source / sim)
    #     reward: torch.Tensor,       # 模拟域 N×1 or N
    ...
    # ):
        ...
        #     # visualization: 
        #     # base_save_dir = "/home/local/ASURITE/longchao/Desktop/project/sim2realFoun/Sim2Real/Sim2RealFoundationPolicy/hilp_zsrl/exp_local/0rebuttal/vis/savegravity_tar"
        #     # if not os.path.exists(base_save_dir):
        #     #     os.makedirs(base_save_dir)
        #     # # visualize_states_and_embeddings(
        #     # #     obs=obs,             # 当前 rollout 的原始状态
        #     # #     enc=enc_sim,         # 对应的 phi(s)
        #     # #     save_dir=base_save_dir,
        #     # #     filename=f"default_gravity_states_phi_{timestamp_str()}.png",
        #     # #     state_xy_idx=(0, 1),           # 如果位置不是前两维，可以改成 (idx_x, idx_y)
        #     # #     title_prefix="default gravity"
        #     # # )
        #     # gravity_tag = f"g4_{timestamp_str()}"  # 自己在 cfg 里写，比如 'g0', 'g-5' 等
        #     # save_embeddings_for_vis(
        #     #     enc_sim=enc_targ,
        #     #     save_dir=base_save_dir,
        #     #     tag=gravity_tag,
        #     # )
    ...
    def infer_meta_from_obs_and_rewards_sim2real(
        ...
        next_obs_targ: torch.Tensor,
        vis: bool = False,
        algo_mode: str = "Direct",
        lambda_wls_set: float = 4.0,
    ):
        if algo_mode == "Direct":
            with torch.no_grad():
                obs = self.encoder(obs)
