...
class FBDDPGAgent:
    ...
    def __init__(self, **kwargs: tp.Any):
        ...
        if cfg.debug:
            ...
            self.backward_net = BackwardMap(
                self.obs_dim, cfg.z_dim, cfg.backward_hidden_dim, norm_z=cfg.norm_z
            ).to(cfg.device)
        ...
        self.actor_success: tp.List[
            float
        ] = []  # only for debugging, can be removed eventually
    ...
    """
    Modify how the goal vector is converted into the latent space. For instance, 
    you could add task-specific transformations to goal_array before passing it to backward_net
    ...
    def get_goal_meta(
        ...
        desired_goal = (
            torch.tensor(goal_array, dtype=torch.float32)
            .unsqueeze(0)
            .to(self.cfg.device)
        )
        with torch.no_grad():
    ...
    """
    The method computes z by interacting with observations (obs), rewards (reward), and the backward network (backward_net)
    ...
    #     with torch.no_grad():
    #         B = self.backward_net(obs) # Compute phi(s), B corresponds to the feature representations
    ...
    def infer_meta_from_obs_and_rewards(
        ...
        # z = torch.matmul(reward.T, B) / reward.shape[0] # 
        # added
        C_phi = torch.matmul(B.T, B) / B.shape[0]  # Covariance matrix
        z = torch.linalg.solve(C_phi, torch.matmul(reward.T, B) / reward.shape[0])
    ...
    """
    Modify how z is sampled (e.g., introduce task-specific constraints or new randomization schemes).
    Example: Replace the Gaussian sampling with a predefined task-specific latent vector.
    """
    ...
    def update_meta(
        ...
        if (
            global_step % self.cfg.update_z_every_step == 0
            and np.random.rand() < self.cfg.update_z_proba
        ):
    ...
    def act(self, obs, meta, step, eval_mode) -> tp.Any:
        ...
        if eval_mode:
            ...
            if self.cfg.additional_metric:
                ...
                F_rand_s = self.forward_net(
                    obs, z, torch.zeros_like(action).uniform_(-1.0, 1.0)
                )
                Qs = [
                    torch.min(*(torch.einsum("sd, sd -> s", F, z) for F in Fs))
                    for Fs in [F_mean_s, F_rand_s]
                ]
    ...
    def update_fb(
        ...
        off_diag = ~I.bool()
        fb_offdiag: tp.Any = 0.5 * sum(
            (M - discount * target_M)[off_diag].pow(2).mean() for M in [M1, M2]
        )
        ...
        if self.cfg.q_loss:
            with torch.no_grad():
                next_Q1, nextQ2 = [
                    torch.einsum("sd, sd -> s", target_Fi, z)
                    for target_Fi in [target_F1, target_F2]
                ]
                next_Q = torch.min(next_Q1, nextQ2)
                cov = torch.matmul(B.T, B) / B.shape[0]
                inv_cov = torch.inverse(cov)
                implicit_reward = (torch.matmul(B, inv_cov) * z).sum(
                    dim=1
                )  # batch_size
                target_Q = (
                    implicit_reward.detach() + discount.squeeze(1) * next_Q
                )  # batch_size
        ...
        if self.cfg.use_tb or self.cfg.use_wandb:
            ...
            if self.cfg.q_loss:
                metrics["q_loss"] = q_loss.item()
            eye_diff = torch.matmul(B.T, B) / B.shape[0] - torch.eye(
                B.shape[1], device=B.device
            )
    ...
    def update(self, replay_loader: ReplayBuffer, step: int) -> tp.Dict[str, float]:
        ...
        if self.cfg.mix_ratio > 0:
            ...
            if not self.cfg.rand_weight:
                ...
                weight = torch.rand(size=(mix_idxs.shape[0], self.cfg.batch_size)).to(
                    self.cfg.device
                )
        ...
        if self.cfg.future_ratio > 0:
            ...
            future_idxs = np.where(
                np.random.uniform(size=self.cfg.batch_size) < self.cfg.future_ratio
            )
        ...
        utils.soft_update_params(
            self.forward_net, self.forward_target_net, self.cfg.fb_target_tau
        )
        utils.soft_update_params(
            self.backward_net, self.backward_target_net, self.cfg.fb_target_tau
        )
