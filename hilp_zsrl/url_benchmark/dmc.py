...
class EnvWrapper:
    ...
    def reset(self) -> TimeStep:
        ...
        # if hasattr(time_step, 'physics'):
        # print("-----time_step.physics-----")
        # print(time_step.physics)
...
class FrameStackWrapper(EnvWrapper):
    def __init__(self, env: Env, num_frames: int, pixels_key: str = "pixels") -> None:
        ...
        self._obs_spec = specs.Array(
            np.concatenate([[pixels_shape[2] * num_frames], pixels_shape[:2]], axis=0),
            np.int8,
            name="observation",
        )
...
def _make_jaco(
    obs_type, domain, task, frame_stack, action_repeat, seed, image_wh=64
...
def make(
    ...
    env = make_fn(
        obs_type, domain, task, frame_stack, action_repeat, seed, image_wh=image_wh
    )
