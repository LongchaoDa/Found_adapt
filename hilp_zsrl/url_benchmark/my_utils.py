...
try:
    ...
    try:
        from moviepy import editor as mpy  # fallback for older layouts
    except Exception:
        mpy = None
import pathlib
...
def save_video(label, tensor, fps=15, n_cols=None):
    if mpy is None:
        raise ModuleNotFoundError(
            "moviepy is required for video saving. Install with: pip install moviepy"
        )
    ...
    plot_path = work_dir / "plots" / f"{label}.mp4"
...
class FigManager:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        plot_path = (
            pathlib.Path(self.eval_dir)
            / "plots"
            / f"{self.label}_{self.global_frame}.png"
        )
        plot_path.parent.mkdir(parents=True, exist_ok=True)
