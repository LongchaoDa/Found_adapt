...
def relabel_episode(
    env: tp.Any, episode: tp.Dict[str, np.ndarray]
...
class EpisodeBatch(tp.Generic[T]):
    ...
    def to(self, device: str) -> "EpisodeBatch[torch.Tensor]":
        ...
        for field in dataclasses.fields(self):
            ...
            if field.name == "meta":
                out[field.name] = {
                    x: torch.as_tensor(y, device=device) for x, y in data.items()
                }  # type: ignore
    ...
    @classmethod
    def collate_fn(
        cls, batches: tp.List["EpisodeBatch[T]"]
    ) -> "EpisodeBatch[torch.Tensor]":
        """Creates a new instance from several by stacking in a new first dimension
        for all attributes
        """
        out: tp.Dict[str, tp.Any] = {}
        if isinstance(
            batches[0].obs, np.ndarray
    ...
    def with_no_reward(self: B) -> B:
        reward = self.reward
        reward = (
            torch.zeros_like(reward) if isinstance(reward, torch.Tensor) else 0 * reward
        )
...
class ReplayBuffer:
    def __init__(
        ...
        self._current_episode: tp.Dict[str, tp.List[np.ndarray]] = (
        ...
        self._batch_names = set(
            field.name for field in dataclasses.fields(ExtendedTimeStep)
        )
    ...
    def _backward_compatibility(self):
        if self._storage and not hasattr(self, "_episodes_length"):
            ...
            assert (
                self._episodes_length[: len(self)].min()
    ...
    def add(self, time_step: TimeStep, meta: tp.Mapping[str, np.ndarray]) -> None:
        ...
        if time_step.last():
            ...
            self._episodes_length[self._idx] = (
                len(self._current_episode["discount"]) - 1
            )  # compensate for the dummy transition at the beginning
            if (
                self._episodes_length[self._idx] != self._episodes_length[self._idx - 1]
            ...
            for i in range(self._frame_stack):
                rets.append(
                    self._storage[storage_key][
                        ep_idx, np.maximum(0, step_idx - (self._frame_stack - i - 1))
                    ]
                )
    ...
    def sample(
        self,
        batch_size,
        custom_reward: tp.Optional[tp.Any] = None,
        ...
        if self._is_fixed_episode_length:
            ...
            if self._episodes_selection_probability is None:
                self._episodes_selection_probability = (
                    self._episodes_length / self._episodes_length.sum()
                )
            ep_idx = np.random.choice(
                np.arange(len(self._episodes_length)),
                size=batch_size,
        ...
        meta = {
            name: data[ep_idx, step_idx - 1]
            for name, data in self._storage.items()
            if name not in self._batch_names
        }
        ...
        if custom_reward is not None:
            reward = np.array(
                [[custom_reward.from_physics(p)] for p in phy], dtype=np.float32
            )
        else:
        ...
        if self._future < 1:
            ...
            future_obs = np.where(
                ...
                ).reshape(future_obs.shape[0], *[1] * (len(future_obs.shape) - 1)),
                curr_obs,
                future_obs,
            )
            random_obs = self.get_obs("observation", random_ep_idx, random_step_idx - 1)
            future_obs = np.where(
                (np.random.rand(batch_size) < self._p_randomgoal).reshape(
                    future_obs.shape[0], *[1] * (len(future_obs.shape) - 1)
                ),
                random_obs,
                future_obs,
            )
