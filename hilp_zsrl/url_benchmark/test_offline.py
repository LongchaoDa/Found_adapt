...
class Config:
    ...
    # align gravity - 44
    load_replay_buffer_env_target: tp.Optional[str] = ( None
        # f"{parameters['config']['path_exorl_learn_24_true_RMSPROP']}/datasets/walker/rnd/replay.pt"  # from the command
...
def _init_eval_meta(workspace, custom_reward: BaseReward = None):
    ...
    save_diff_dir = PROJECT_DIR / "a_analysis_25" / "diff"
    save_diff_dir.mkdir(parents=True, exist_ok=True)

    # get the most close obs (sample 10)
    if show_obs_alignment:
        get_obs_difference(obs, obs_env_targ, save_path=str(save_diff_dir / "obs_diff.png"), num_samples=10)
        get_obs_difference(obs_t, obs_t_env_targ, save_path=str(save_diff_dir / "obs_diff_t.png"), num_samples=10)
